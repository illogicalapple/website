export default class Spelchek() {
	constructor(text) {
		...
		this.warp = {
			...
			vowel(priority) {
				if(!priority) this.text = this.text.split("").map(element => this._vowels.includes(element) ? this._vowels(Math.floor(Math.random() * 5)) : element).join("");
				return { PRIORITY: 200 }
			},
			typo(priority) {
				if(!priority) this.text = this.text.split("").map(element => this._alphabet.includes(element) && Math.random() < 0.09 ? this._alphabet[Math.floor(Math.random() * 54)] : element);
				return { PRIORITY: 201 }
			},
			punctuation(priority) {
				if(!priority) this.text = this.text.split("").filter(e => !this._punctuation.includes(e)).forEach((e, i, a) => Math.random() < 0.09 && a.splice(i, 0, this._punctuation[Math.random() * this._punctuation.length]));
				return { PRIORITY: 199 }
			},
			snake(priority) {
				if(!priority) this.text = this.text.split("").filter(e => Math.random() > 0.9);
				return { PRIORITY: 202 }
			},
